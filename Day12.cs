namespace AoC{
    class Day12 {
        public static void Part1() {
            ...
            //process the input
            Console.WriteLine(findPaths(new List<string>(), nodes["start"], nodes));
        }

        static int findPaths(List<string> _visited, Node n, Dictionary<string, Node> d){
            int total = 0;
            ...
            if(n.name == "end") return 1;
                ...
                for(int i = 0; i < n.connections.Count(); i++){
                    if (!visited.Contains(n.connections[i])){
                        total += findPaths(new List<string>(visited), d[n.connections[i]], d);
                    }
                }
            }
            return total;
